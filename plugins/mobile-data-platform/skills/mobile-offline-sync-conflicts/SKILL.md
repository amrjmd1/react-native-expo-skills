...
description: Offline sync architecture guidance for mobile apps. Use for local-first state, sync queue design, retries, conflict detection/resolution, idempotency, and recovery from partial sync failures.
---

# Offline Sync and Conflict Control

## Mission
Deliver resilient offline-first behavior with deterministic conflict handling.

## Workflow
1. Define local source-of-truth and sync queue semantics.
2. Implement idempotent mutations and retry windows.
3. Detect and resolve conflicts with explicit policy.
4. Provide reconciliation and user-visible conflict UX.
